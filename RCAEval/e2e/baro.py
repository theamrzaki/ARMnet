...
def baro(
    data, inject_time=None, dataset=None, num_loop=None, sli=None, anomalies=None, with_baro=False, seq_len=1, **kwargs
):
    seq_len=12
    print(f"with_baro={with_baro}")
    ...
    for col in normal_df.columns:
        ...
        # Reshape for sequence length if provided
        if seq_len > 1:
            ...
            # collapse sequence dimension (mean or flatten)
            a = a_seq.mean(axis=1)  # or a_seq.flatten()
            b = b_seq.mean(axis=1)
